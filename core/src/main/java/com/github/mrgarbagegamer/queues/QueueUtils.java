...
public final class QueueUtils {
    ...
    private static <G, M> void requireValidArguments(List<? extends QueueWrapper<G>> gtmQueues,
        ...
        // Build two QueueGroups to validate selectors
        final QueueGroup<G> gtmGroup = QueueGroup.newGtmGroup(gtmQueues, generatorOfferSelector,
                monkeyPollSelector, config);
        final QueueGroup<M> mtgGroup = QueueGroup.newMtgGroup(mtgQueues, monkeyOfferSelector,
                generatorPollSelector, config);

        // Validate selectors:
        gtmGroup.validateSelectors();
        mtgGroup.validateSelectors();
    }
